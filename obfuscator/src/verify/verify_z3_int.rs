...
pub fn verify_int(file_path: &Path) {
    let mut init_code = String::new();
    let mut init_file = File::open("./z3/z3_init_int.txt").unwrap();
    ...
    for (origin, obfuscation, _id) in formulas {
        ...
        stdin.write_all(format!("(declare-fun x () Int)\n").as_bytes()).unwrap();
        stdin.write_all(format!("(declare-fun y () Int)\n").as_bytes()).unwrap();
